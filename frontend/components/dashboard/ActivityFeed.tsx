  ...
  return (
    <div className="card-bento p-5">
      ...
      {isLoading ? (
        ...
        <ol className="relative border-l border-hairline ml-3 space-y-0">
          {logs.map((log, i) => {
            ...
            return (
              <li key={log.id} className={cn('ml-4 pb-4', isLast && 'pb-0')}>
                {/* Timeline dot — border matches card bg */}
                <span className={cn(
                  'absolute -left-[5px] w-2.5 h-2.5 rounded-full border-2 border-canvas',
                  cfg.dot,
                )} />
